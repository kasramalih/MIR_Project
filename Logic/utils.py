...
def search(
    ...
    return search_engine.search(
        query, method, weight_dic, max_results=max_result_count, safe_ranking=True, smoothing_method=smoothing_method, alpha=alpha, lamda=lamda
    )
...
def get_movie_by_id(id: str, movies_dataset: List[Dict[str, str]]) -> Dict[str, str]:
    ...
    result["Image_URL"] = (
        image_URL[id] if id in image_URL.keys() else "https://m.media-amazon.com/images/M/MV5BNDE3ODcxYzMtY2YzZC00NmNlLWJiNDMtZDViZWM2MzIxZDYwXkEyXkFqcGdeQXVyNjAwNDUxODI@._V1_.jpg"
    )
    result["URL"] = (
